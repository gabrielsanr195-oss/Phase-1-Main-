...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : 'Login fallido');
    } finally {
      setLoading(false);
    }
  }
...
const s: Record<string, React.CSSProperties> = {
  page: { minHeight: '100vh', backgroundColor: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  card: { backgroundColor: '#1a1a1a', borderRadius: '12px', padding: '2rem', width: '100%', maxWidth: '360px', color: '#f0f0f0' },
  title: { fontSize: '1.3rem', fontWeight: 700, margin: '0 0 1.5rem', color: '#9c6eff' },
  form: { display: 'flex', flexDirection: 'column', gap: '0.4rem' },
  label: { fontSize: '0.78rem', color: '#888', marginTop: '0.5rem' },
